...
import com.mkumar.ui.screens.customer.model.ContactLensFormState
import androidx.compose.foundation.text.KeyboardOptions

@Composable
fun ContactLensForm(
    state: ContactLensFormState,
    onChange: (ContactLensFormState) -> Unit,
) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        OutlinedTextField(
            value = state.brand,
            onValueChange = { onChange(state.copy(brand = it)) },
            label = { Text("Brand") },
