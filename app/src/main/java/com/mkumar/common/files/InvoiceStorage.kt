...
fun saveInvoicePdf(context: Context, fileName: String, bytes: ByteArray): Uri {
    val values = ContentValues().apply {
        ...
        put(MediaStore.Downloads.RELATIVE_PATH, RELATIVE_DIR) // Shows under Files > Downloads > Documents > MKumar > Invoices
        put(MediaStore.Downloads.IS_PENDING, 1)
    }
    val uri = context.contentResolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, values)
        ?: error("Failed to insert invoice in MediaStore")

    context.contentResolver.openOutputStream(uri)?.use { it.write(bytes) }
        ?: error("Failed to open OutputStream for invoice")
